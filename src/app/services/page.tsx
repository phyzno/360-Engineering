...
export default function ServicesPage() {
  ...
  return (
    <PageTransition>
      <section className="hero-padding bg-[#0f1a0a] border-b border-[#1a2912]">
      ...
      <section className="bg-[#0a1206]">
        {services.map((service, index) => (
          <div 
            key={service.id} 
            id={service.id}
            className={`grid grid-cols-1 md:grid-cols-2 min-h-[70vh] border-b border-[#1a2912] ${
          ...
          >
            ...
            <div className="flex flex-col justify-center p-12 md:p-24 lg:p-32">
              <span className="text-[#c9a84c] tracking-[0.2em] uppercase text-sm font-semibold mb-6 block">
                0{index + 1}
              </span>
              <h2 className="font-heading text-4xl text-[#f5f0e8] mb-6">{service.title}</h2>
              <p className="text-[#9ba89e] leading-relaxed text-lg">
                {service.desc}
              </p>
            </div>
          </div>
